...
// So the program connects directly to the localhost (L.S.)
...
"use strict"
function GameState() {
    // when we call the newWall in AL, we need to know the utterance (L.S.)
    this.AL_Game = null
    this.AL_utterance = "";
...
function newWall(gs) {
    var ind = 5; // Triangle level index (L.S.)
    if (GS.getSuccessCount(configs.levels[ind].id) == configs.levels[ind].minSuccess) { // Auto-reset after the game is finished (L.S.)
      window.location.href="../Intro.html", true;
      document.getElementById("reset").click();
    }

    var wallcommand = "(execute (call edu.stanford.nlp.sempre.cubeworld.StacksWorld.getLevel (string {task})))"
	._format({task: gs.AL_flag+configs.levels[gs.taskind].id}); // attach arguments here!
  ...
  if (gs.AL_Game == null) { // We just need to now if we are using AL, on the beginning
    if (JSON.parse(jsonstr)['AL_Game'] == "false") {
      gs.AL_Game = false;
      document.getElementById("explicitButton").style.visibility = "hidden";
    }
    else {
      gs.AL_Game = true;
    }
  ...
  if (JSON.parse(jsonstr)['activeLearning'] == "true") { // (L.S.)
    gs.taskind = 7;
    gs.activeLearning = true;
    gs.AL_utterance =  JSON.parse(jsonstr)['AL_utterance'];
    activeLearning(gs);
    newWall(gs);
    return;
  }
  ...
  if (gs.activeLearning == true) { // (L.S.)
    document.getElementById("maintextarea").value = gs.AL_utterance;
    runCurrentQuery(gs);
  }
    })
}
...
var GameAction = {
	...
	sempre.sempreQuery({q: gs.query+"%end%", accept:gs.NBest[gs.NBestInd].rank, sessionId:gs.sessionId}, function(){})
    },
  ...
  // Trigger the active learning every 3 iterations (L.S.)
  if ((gs.AL_itter != 0 && gs.AL_itter == 2) || (gs.AL_itter > 3 && gs.AL_itter % 3 == 0)) { 
    gs.AL_flag = "%end%";
  }
  gs.AL_itter++;
...
function popTasks() {
    ...
    for (var l in configs.levels) {
      if(configs.levels[l].id != "activeLearning" && configs.levels[l].id != "babynot") {
...
function acceptOnclick() {
    if (GS.activeLearning == true) { // Isolated accept for the AL (L.S.)
  GameAction.nextLevel(GS)
  ga('send', 'event', "custom", "passedlevel", GS.taskInd);
    }
    else if (GameAction.checkAnswer(GS)) {
	GameAction.nextLevel(GS)
	ga('send', 'event', "custom", "passedlevel", GS.taskInd);
    } else {
...
function explicitMenu() { //(L.S.)
  window.location.href="explicit/explicit_colorPos.html?sessionId="+GS.sessionId, true
}

function activeLearning(gs) { //(L.S.)
  visib = gs.activeLearning == true ? "hidden" : "visible";
  disp = gs.activeLearning == true ? "none" : "initial";

  document.getElementById("tasks").style.visibility = visib;
  /*document.getElementById("reset").style.visibility = visib;
  document.getElementById("bitsPenalty").style.visibility = visib;*/
  document.getElementById("explicitButton").style.visibility = visib;
  document.getElementById("dobutton").style.display = disp;
  document.getElementById("undobutton").style.display = disp;
  document.getElementById("goalblocks").style.visibility = visib;
  document.getElementById("maintextarea").style.display = disp;
  document.getElementById("helptext").style.display = disp;
  document.getElementById("container").style.marginTop = gs.activeLearning == true ? "80px" : "0px";
  document.getElementById("ALtitle2_2").textContent = "\"" + gs.AL_utterance + "\"";
  document.getElementById("ALtitle").style.display = gs.activeLearning == true ? "initial" : "none";
  document.getElementById("notAction").style.display = gs.activeLearning == true ? "initial" : "none";
  document.getElementById("container").style.width = gs.activeLearning == true ? "800px" : "950px";
}

// So when the user returns from another menu, does not start from the beginning (L.S.)
function goodInitialization() {
  for (var i = 0; i < configs.levels.length; i++) {
    if (GS.getSuccessCount(configs.levels[i].id) < configs.levels[i].minSuccess) {
      GS.taskind = i;
      break;
    }
  }
