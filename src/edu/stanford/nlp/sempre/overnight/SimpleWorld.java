...
public final class SimpleWorld {
  ...
  public static List<Value> domain(String property) {
    createWorld();
    if (opts.ignoreCase) property = property.toLowerCase();
  ...
  private static void checkType2(String property, Value e2) {
    ...
    if (!getType(e2).startsWith("@descstr_") && !getType(e2).equals(type2))
  ...
  public static List<Value> sortListByProperty(List<Value> entities, String mode, String property) {
	List<Value> properties = getProperty(entities, property);
    return sortList(properties, mode);
  }
  
  public static List<Value> sortList(List<Value> entities, String mode) {
	Comparator<Value> ctor;
	if (mode.equals("dec")) {
	  ctor = new Comparator<Value>() {
		@Override
		public int compare(Value i1, Value i2) {
		  return getDouble(i1) >= getDouble(i2) ? -1 : 1;
		}};
	} else {
	  ctor = new Comparator<Value>() {
		@Override
		public int compare(Value i1, Value i2) {
		  return getDouble(i1) > getDouble(i2) ? 1 : -11;
		}};
	}
    Collections.sort(entities, ctor);
    return entities;
  }
  ...
  public static List<Value> superlative(List<Value> entities, String mode, String property) {
    ...
    for (Value e : entities) {
      ...
      checkType1(property, e);
      if (bestEntities == null || bestDegree == Double.NaN || (mode.equals(MAX) ? degree > bestDegree : degree < bestDegree)) {
  ...
  public static List<Value> setOperations(List<Value> s1, List<Value> s2, String mode) {
	Set<Value> retset = new HashSet<Value>(s1);
    ...
    return new ArrayList<Value>(retset);
  }
  ...
  public static List<Value> aggregate(String mode, List<Value> values) {
    ...
    switch (mode) {
      ...
      case "mode": result = sum; break;
      case "median": result = sum; break;
  ...
  public static List<Value> subList(List<Value> values, Value startvalue, List<Value> endvalue) {
	return subList(values, startvalue, endvalue.get(0));
  }
  public static List<Value> subList(List<Value> values, List<Value> startvalue, Value endvalue) {
	return subList(values, startvalue.get(0), endvalue);
  }
  public static List<Value> subList(List<Value> values, Value startvalue, Value endvalue) {
   	int start = (int)getDouble(startvalue);
	int end = (int)getDouble(endvalue);
	if (start == -1) {
	  int size = values.size();
	  return values.subList(size - end, size); 
	}
	else {
 ...
 public static List<List<NumberValue>> initialWall(String wallname) {
   List<NumberValue> stack1 = new ArrayList<>();
   stack1.add(new NumberValue(1));
   stack1.add(new NumberValue(2));
   stack1.add(new NumberValue(3));
   stack1.add(new NumberValue(4));
   
   List<NumberValue> stack2 = new ArrayList<>();
   stack2.add(new NumberValue(4));
   stack2.add(new NumberValue(2));
   stack2.add(new NumberValue(3));
...
// just attaches some information via the formula without changing the value
 public static List<Value> attachInfo(String info, Value inObject) { return attachInfo(info, singleton(inObject)); }
 public static List<Value> attachInfo(String info, List<Value> inObjects) {
   return inObjects;
 }
    ...
    for (Value obj : inObjects) {
      ...
      for (Value v : values) {
        if (!opts.joinDedup || !outObjectsCache.contains(v)) {
          if (obj instanceof NameValue && v instanceof NumberValue) {
        	String desc = ((NameValue)obj).id;
        	double value = ((NumberValue)v).value;
        	v = new NumberValue(value, "@descstr_" + desc);
          } 
          else if (obj instanceof NameValue && v instanceof NameValue) {
        	String desc = ((NameValue)obj).id;
        	String value = ((NameValue)v).id;
        	v = new NameValue(value, desc);		
          } 
          outObjects.add(v);
  ...
  /////////////////// dotaOp
  private static double dotaOp(String dotaop, double v) {
    switch (dotaop) {
      case "rolldist_mana": return (v-15.0-v*0.07)/(v*0.007+12)*100.0;
      case "mana_rolldist": return (15+0.12*v)/(0.93-0.01*0.007*v);
      case "dmgreduction_armor": return 0.06*v / (1+0.06*v);
      case "dmgtaken_armor": return 1-0.06*v / (1+0.06*v);
      case "ehp_armor": return 1/(1-0.06*v / (1+0.06*v));
      case "dmgincrease_negarmor": return 1-0.06*Math.abs(v) / (1+0.06*Math.abs(v));
      default: throw new RuntimeException("Invalid operation: " + dotaop);
    }
  }
  public static List<Value> dotaOp(String dotaop, Value v) {
    return singleton(new NumberValue(dotaOp(dotaop, getDouble(v))));
  }
  public static List<Value> dotaOp(String dotaop, List<Value> args1) {
    List<Value> result = new ArrayList<Value>();
    for (Value v : args1)
      result.addAll(dotaOp(dotaop, v));
    return result;
  }
  
  public static TableValue getDescription(List<Value> es) {
	Value e = es.get(0);
	createWorld();
	//return singleton(new StringValue(entitydescriptions.toString()));
    List<Pair<String,Value>> propvalue = entitydescriptions.get(e);
    
    List<String> header = new ArrayList<String>();
    header.add("property"); header.add("value"); 
    List<List<Value>> rows = new ArrayList<List<Value>>();
    for(Pair<String,Value> p : propvalue) {
      if(!p.getFirst().equals("type"))
      {
    	List<Value> pairvalue = new ArrayList<Value>();
    	pairvalue.add(new StringValue(p.getFirst()));
    	pairvalue.add(p.getSecond());
    	rows.add(pairvalue);
      }
    }
    TableValue result = new TableValue(header, rows);
    return result; 
  }
  ...
  private static Map<Value, List<Pair<String, Value>>> entitydescriptions;  // property value pairs keyed by entities of (e1, (property, e2)) triples
  ...
  private static void insertDB(Value e1, String property, Value e2) {
    ...
    MapUtils.addToList(entitydescriptions, e1, new Pair(property, e2));
    MapUtils.addToList(entitydescriptions, e2, new Pair(reverse(property), e1));
  }
