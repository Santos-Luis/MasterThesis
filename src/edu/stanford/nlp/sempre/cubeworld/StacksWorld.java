...
public final class StacksWorld {
  ...
  public static final Random rand = new Random(2); //(L.S.) in order to get dif. setups of 
                                                  //initial states in dif. explicit feedbacks
  ...
  // Transform a string of states in a ArrayList, where each position contains a column (L.S.)
  public static ArrayList<String> statesToArray(String states) {
  ...
  public static String getLevel(String level) {
    ...
    /* #################
     *  To the AL menu, we need a initial predefined state and a empty final state
     *
     *  Levels so we can work with the explicit learning
     *  1-normal (color OR position) | 2-mixed (color_position)
     *
     *  (L.S.)
     * ################# */
    if(level.startsWith("activeLearning")) {
      ArrayList<String> statesArray = statesToArray(level.substring(level.indexOf("["), level.lastIndexOf("]")+1));
      wall = new ArrayList<>();
      target = new ArrayList<>();
      int numStack = statesArray.size();
      
      for (int s = 0; s < numStack; s++) {
        List<NumberValue> stack = new ArrayList<>();
        String[] parts = statesArray.get(s).split(",");
        for (int i = 0; i < parts.length; i++) {
          stack.add(new NumberValue(Double.parseDouble(parts[i]), "COLOR"));
        }
        wall.add(stack);
      }
      for (int s = 0; s < numStack; s++) {
    ...
    else if(level.startsWith("explicit_1")) {
      wall = new ArrayList<>();
      target = new ArrayList<>();
      int numStack = 4;
      ...
      for (int s = 0; s < numStack; s++) {
        List<NumberValue> stack = new ArrayList<>();
        stack.add(new NumberValue(s, "COLOR"));
        wall.add(stack);
      }
      for (int s = 0; s < numStack; s++) {
        List<NumberValue> stack = new ArrayList<>();
    ...
    else if(level.startsWith("explicit_2")) {
      wall = new ArrayList<>();
      target = new ArrayList<>();
      int numStack = 4;

      for (int s = 0; s < numStack; s++) {    // In order to generate 4 random states with diff colors
        List<NumberValue> stack;
        if(s == 0) {
          stack = new ArrayList<>();
          stack.add(new NumberValue(1, "COLOR"));
        } else {
          stack = new ArrayList<>();
          stack.add(new NumberValue(rand.nextInt(4), "COLOR"));
          while(wall.contains(stack)) {
            stack.remove(0);
            stack.add(new NumberValue(rand.nextInt(4), "COLOR"));
          }
        }
  ...
  public static List<List<NumberValue>> getWallFromContext(ContextValue context) {
    ...
    try {
      ...
      return stringToWall("[[1,2,3],[2,1,2,1],[2]]");
    }
  }
  //wallString consists of [[1,2,3],[2,1,2],[2,3,4]]
