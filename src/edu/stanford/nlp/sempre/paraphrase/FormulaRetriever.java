...
public class FormulaRetriever {
  ...
  public static class Options {
    ...
    public EntityLexicon.SearchStrategy entitySearchStrategy;
  }
  ...
  public List<FormulaGenerationInfo> retrieveFormulas(ParsingExample ex) {
    ...
    List<FormulaGenerationInfo> res = new ArrayList<>();
    try {
  ...
  private List<FormulaGenerationInfo> createUnaries(LanguageInfo lInfo, FormulaGenerationInfo inFgInfo) {
    ...
    List<FormulaGenerationInfo> res = new ArrayList<FormulaGenerationInfo>();
    Set<String> subtypes = SemTypeHierarchy.singleton.getSubtypes(inFgInfo.bInfo.expectedType1);
    for (String subtype : subtypes) {
      if (subtype.equals(inFgInfo.bInfo.expectedType1)) continue;  // Don't include itself
      if (badDomain(subtype)) continue;
  ...
  private boolean toFilter(BinaryFormulaInfo bInfo,
    ...
    if (opts.filterRelations && ParaphraseUtils.isInteger(entityEntry.textDescription))
      return true;
    if (opts.filterRelations && CanonicalNames.PRIMITIVES.contains(expectedType1))
      return true;
    return false;
  }
  ...
  private List<EntityLexicalEntry> getEntityEntries(String phrase)
    ...
    List<EntityLexicalEntry> res = (List<EntityLexicalEntry>) lexicon.lookupEntities(phrase, opts.entitySearchStrategy);
