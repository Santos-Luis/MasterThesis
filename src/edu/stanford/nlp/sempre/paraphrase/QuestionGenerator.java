...
public class QuestionGenerator {
  ...
  private final Map<Formula, Pair<String, Double>> formulaToLexemsMap = new HashMap<>();
  private final String lexiconFile = "lib/fb_data/6/binaryInfoStringAndAlignment.txt";

  private final Map<String, Annotation> lingAnnotationCache = new HashMap<>();
  ...
  private String generateNonCvtQuestion(FormulaGenerationInfo fgInfo, String binaryDesc,
    ...
    if (binaryDesc.endsWith("here")) { // special type of description that behaves weirdly
      ...
      if (t.children().length == 2 && t.getChild(0).label().toString().equals("NP") &&
        ...
        throw new RuntimeException("Unhandled S node: " + t);
      }
      ...
      throw new RuntimeException("Not handling " + fgInfo.bInfo + ", category=" + category);
    }
    return res;
  }
  ...
  private String generateCvtQuestion(FormulaGenerationInfo fgInfo,
    ...
    if (!isReversed) {
      ...
      // other
        ...
        throw new RuntimeException("Does not handle: " + fgInfo.bInfo + ", root1=" + root1 + ", root2=" + root2 + ", is reversed=false");
    } else {
      if (isNPorFRAG(root1) && isNPorXorFRAG(root2)) {
        if (CanonicalNames.PRIMITIVES.contains(fgInfo.bInfo.expectedType1))
      ...
      // handle things that start with PP
        ...
        throw new RuntimeException("Does not handle: " + fgInfo.bInfo + ", root1=" + root1 + ", root2=" + root2 + ", is reversed=true");
    }
    if (opts.verbose >= 3)
