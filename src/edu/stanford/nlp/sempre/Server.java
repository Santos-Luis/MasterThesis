...
public class Server {
  ...
  class ExchangeState {
    ...
    HtmlElement makeDerivationHelper(Example ex, Derivation deriv, String indent, boolean moreInfo) {
      ...
      String description = "";
      if (deriv.start != -1)
        description = cat + "[&nbsp;" + H.span().child(ex.phraseString(deriv.start, deriv.end)).cls("word") + "]" + " &rarr; " + deriv.formula;
      else
        description = cat + "[&nbsp;" + H.span().child("Floating").cls("word") + "]" + " &rarr; " + deriv.formula;
    ...
    String makeLisp2json(Master.Response response) {
      ...
      for (Derivation deriv : response.getExample().getPredDerivations()) {
        Map<String, Object> item = new HashMap<String, Object>();
        Value value = deriv.getValue();
        item.put("value", convertLispToList(value.toLispTree()));
        item.put("score", deriv.score);
        item.put("prob", deriv.prob);
        item.put("pprob", deriv.pragmatic_prob);
    ...
    void handleQuery(String sessionId) throws IOException {
      ...
      boolean endIteration = false; // If the user entered the last feedback of an iteration (L.S.)
      if (query.contains("%end%")) { // The following code cannot have this tag, but we need it later (L.S.)
      ...
      if (endIteration) // Bringing back the previously deleted tag (L.S.)
        query += "%end%";
      ...
      if (masterResponse != null) {
        ...
        } else if(jsonFormat() && ex == null) { //a command was executed
          if (masterResponse.commandResponse!=null) {
            Map<String, Object> json = new HashMap<String, Object>();
            json.put("exValue", masterResponse.commandResponse.toString());

            json.put("AL_Game", ""+master.opts.activeLearningGame);

            // Passing the utterance selected for be asked in the active learning (L.S.)
            boolean activeLearning = master.opts.activeLearningNow;
            if (activeLearning) {
              json.put("activeLearning", ""+activeLearning);
              json.put("AL_utterance", master.activeLearningUtterance);
              master.opts.activeLearningNow = false;
            }
